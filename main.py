...
# --- ฟังก์ชันสร้าง Embed สำหรับผลการค้นหาทีม ---
...
class PlayerSearchModal(Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not players_found: await interaction.followup.send(f"ไม่พบนักแข่งชื่อ '{player_name}'", ephemeral=True); return
        player_data = players_found[0]
        if not player_data.get('current_team'): await interaction.followup.send(f"**{player_data['name']}** ไม่ได้สังกัดทีม", ephemeral=True); return
...
class TeamSearchModal(Modal):
    def __init__(self, game_slug: str):
        ...
        self.add_item(InputText(label="ชื่อทีม", placeholder="เช่น Paper Rex, Fnatic, T1", required=True))
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not teams_found:
            await interaction.followup.send(f"ไม่พบทีมชื่อ '{team_name}' ในระบบของเกมนี้ครับ", ephemeral=True)
            return
            
        team_data = teams_found[0]
...
class MainControlPanelView(View):
    ...
    async def schedule_button(self, button: Button, interaction: discord.Interaction):
        await interaction.response.send_message(content="เลือกช่วงเวลาที่ต้องการ", view=TimePeriodSelectView(), ephemeral=True)

    @discord.ui.button(label="ค้นหานักแข่ง", style=discord.ButtonStyle.primary, custom_id="persistent_view:search_player", emoji="🔍")
    async def search_player_button(self, button: Button, interaction: discord.Interaction):
        await interaction.response.send_message(view=GameSelectView(action="search_player"), ephemeral=True)
...
async def setup_panel(ctx: discord.ApplicationContext):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "🔹 **ค้นหานักแข่ง:** กดปุ่ม `ค้นหานักแข่ง` เพื่อดูตารางแข่งของนักแข่ง\n"
            "🔹 **ค้นหาทีม:** กดปุ่ม `ค้นหาทีม` เพื่อดูตารางแข่งของทีม\n\n"
        ),
        color=0x2ECC71
    );
